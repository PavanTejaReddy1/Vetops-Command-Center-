/**
 * Notification model — PHASE 2 (Mongoose schema not implemented yet).
 *
 * Structure placeholder so the shape of the future schema is discoverable
 * from the codebase. Fields below are illustrative only, based on the
 * dummy data used by the frontend (src/data/notification.js).
 *
 * Example (for Phase 2 reference):
 *
 * import mongoose from 'mongoose';
 *
 * const notificationSchema = new mongoose.Schema(
 *   {
 *     // ...fields specific to Notifications
 *   },
 *   { timestamps: true }
 * );
 *
 * export const Notification = mongoose.model('Notification', notificationSchema);
 */
export const NotificationModel = null; // placeholder export until Phase 2
